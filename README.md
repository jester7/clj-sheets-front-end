...
* [`src/cljsheets/styles.cljs`](src/cljsheets/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/cljsheets/`](src/cljsheets/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljsheets/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `cljsheets` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cljsheets/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cljsheets.example
  (:require [cljsheets.config :as config])
